...
EMax.mixtura <- function (mu.link = "log", sigma.link="log", nu.link = "logit")
{
  mstats <- checklink(   "mu.link", "EMax.mixtura", substitute(mu.link), 
                         c("inverse", "log", "identity", "own"))
  dstats <- checklink("sigma.link", "EMax.mixtura", substitute(sigma.link), 
                      c("inverse", "log", "identity", "own"))
  vstats <- checklink(   "nu.link", "EMax.mixtura", substitute(nu.link),    
                         c("1/nu^2", "log", "identity", "own"))
  structure(
    list(family = c("EMax.mixtura", "EMax.mixtura"),
         ...
         dldm = function(y,mu,sigma,nu){ #----------------------------------------------------- ok
           lpdf<-function(t,x,sigma,nu){log(dauxiEMax.mixtura(t,x,sigma,nu))}
         ...
         dldd = function(y,mu,sigma,nu){#----------------------------------------------------- ok  
           lpdf<-function(t,mu,x,nu){log(dauxiEMax.mixtura(t,mu,x,nu))}
         ...
         dldv = function(y,mu,sigma,nu){#----------------------------------------------------- ok
           lpdf<-function(t,mu,sigma,x){log(dauxiEMax.mixtura(t,mu,sigma,x))}
         ...
         { 
           -2*dEMax.mixtura(y,mu,sigma,nu,log=TRUE)
         } ,                     
         rqres = expression(   
           rqres(pfun="pEMax.mixtura", type="Continuous", y=y, mu=mu, sigma=sigma, nu=nu)) ,
         ...
         mu.valid = function(mu) all(sigma > 0), 
...
#-----------------------------------------------------------------
dEMax.mixtura  <- function(x, mu = 2, sigma = 1, nu = 0.5, log = FALSE){
  ...
  fy1 <- (1-nu)*(sigma*g*(G^(sigma-1)))
  
  if(log==FALSE) fy<-fy1 else fy<-log(fy1)
  fy
}    
#-----------------------------------------------------------------  
pEMax.mixtura <- function(q, mu = 2, sigma = 1, nu = 0.5, lower.tail = TRUE, log.p = FALSE){  
  if (any(mu < 0))  stop(paste("mu must be positive", "\n", "")) 
  if (any(sigma < 0))  stop(paste("sigma must be positive", "\n", ""))  
  if (any(nu < 0))  stop(paste("nu must be positive", "\n", ""))
...
#-----------------------------------------------------------------  
#-----------------------------------------------------------------  
hEMax.mixtura <- function(x, mu = 2, sigma = 1, nu = 0.5, lower.tail = TRUE, log.p = FALSE){  
  hazard=dEMax.mixtura (x,mu,sigma,nu)/(1-pEMax.mixtura (x,mu,sigma,nu))
}
#-----------------------------------------------------------------  


dauxiEMax.mixtura <- function(t,mu,sigma,nu){ 
